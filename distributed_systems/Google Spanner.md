...
# Hierarchy 
A Spanner deployment is called a **universe** (there are few universes). A universe is organized as a set of **zones**, admistriative deployment locations across which data can be replicated. 
...
- Per-zone **location proxies** are used by clients to locate the spanservers.
	- This achieves a simialr purpose as [[Chord]].
...
Each spanserver implements one instance (a single state machine) of [[Paxos Algorithm|Paxos]], storing its metata and log in its corresponding tablet. Leaders live long and have a time-based leader lease (defaults to $10$ seconds). 
...
At every replica there is a **Paxos leader**. The leadeader must implement a **lock table** for concurrency control. The lock table maps ranges of keys to locks states. Operations that synchronization, such as transactional reads, acquire locks in the lock table; other operations bypass the lock table.
...
> 
> It is up to the application to determine what is a transactional read, v.s. a regular read.
...
Spanner exposes a data model based on schematized semi-relational tables, a query language (like SQL), and general purpose transactions. Thes are because of mistakes in previous projects. 
...
- Client applications explicity declare these in the database schema using `INTERLEAVE IN` declarations.
- The table at the very top of the hierarchy is called a **directory table**.
- A row in that directory table, combined with descendent rows in the interleaved tables that start with the exact same key, logically forms a directory.
...
The comamnd `ON DELETE CASCADE` means if a row is deleted, the child rows are deleted. 
...
- client can provide timetamp or bound (and let Spanner choose)
- only executes [[#^215196|on any replica that is sufficiently up-to-date]] ^31c538

## Paxos Leader Leases
Spanner's Paxos implementation uses timed leases to make leadership long-lives. A potential leader sends requests for timed **lease votes**. Upon receiving a [[Quorum]], the leader has a lease. The lease extends on a successful write.
...
- leaders can abdicate before its lease expires by releasing its slave replicas from their least votes
...
Let start and commti events of transaction $T_{i}$ be $e_{i}^{start}$ and $e_{i}^{commit}$. Let the commit timetamp of $T_{i}$ be $s_{i}$. Then 
$$
...
2. **Commit Wait**: Coordinator leader ensures that clients cannot see any data committed by $T_{i}$ until $\texttt{TT.afeter}(s_{i})$ is true. Ensures that $s_{i} < t_{abs}(e_{i}^{commit})$. ^f2d588
...
\end{aligned}
$$
This is how Spanner correctly determines [[#^b608e0|whether a]] [[#^31c538|replica is suffuciently]] [[#^5f7eac|up-to-date]] to satisfy a read. Each replica keeps $t_{safe}$, the maximum timetamp at which a replica is up-to-date, such that a read can occur if $t \leq t_{safe}$. In particular
$$
...
- The coordinator leader ensures that the transaction's commit timetamp $s_{i} \geq s_{i,g}^{prepare}$ over all participant groups $g$. 
...
### Refning TM Safety
A single prepared transaction can prevent $t_{safe}$ from advancing. As a result, no reads can occur at later timestamps, even if they do not conflict. Augmenting $t_{safe}^{TM}$ with a fine-grained mapping form key ranges to prepared-transaction timestamps fixes this, by checking the range on incoming reads.

### Refining Safe Paxos Time During Idleness
If we have no Paxos writes, $t_{safe}^{Paxos}$ will never advance, thus blocking writes. Spanner fixes this by taking advantage of the [[#Paxos Leader Leases|disjointedness of leader-lease intervals]]. Each Paxos leader keeps a mapping called $\texttt{MinNextTS}(n)$ which acts as a threshold promising the minimum timetsamp that could possibly be assigned to the next future write. 
...
- This is because reads return a timestamp, and uncommited writes have not yet been assigned timestamps. 
...
### 2. Prepare Phase
When a client has finished all reads and buffered all writes, it can begin 2PC. The client chooses a one of the Paxos groups to be the "coordinator" and sends a commit message and the buffered writes to the leader of all "participant" groups. Having the client drive the 2PC avoids sending data twice. 

A participant leader (except the coordinator) first acquires write locks for the incoming data. It then chooses a **prepare timestamp** (larger than $s_{max}$) and logs a prepare record through its Paxos state machine. It then notidies the coordinator of this prepare timestamp. 
...
Assigning a timestamp requires a negotiation phase between all of the Paxos groups that are involed in the reads. Spanner requires a **scope** abstraction for every read-only transaction; it is an expression that summarizes the keys to be read. 
...
$\texttt{LastTS}()$ can block a non-conflicting read-only transaction while it waits for the last transaction commit. We can fix this by doing something similar to [[#Refning TM Safety]] by adding key ranges it actually conflicts with. 
...
A non-blocking way to do this is to register these operations in the future TrueTime timetamp $t$. Since it is in the future, the system has time to register the upcoming change across thousands of servers with minimal disruption to other concurrenct activity. 
