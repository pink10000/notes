...
While the time to generate an authenticator and its size grow linearly with the number of replicas, it remains significantly faster and smaller than a 1024-bit RSA signature for typical system configurations (e.g., tolerating up to $f \le 4$ simultaneous faults).
